...
`MeCab`으로 한글 분석이 가능하지만, `MeCab` 프로그램만 설치한다고 해결되는 것이 아니다.
...
`innodb_ft_user_stopword_table` 시스템 변수를 이용할 수도 있는데,
...
`클러스터링 인덱스`는 **테이블의 PK에 대해서만 적용**된다.
...
![image](https://github.com/AK-47-Study/real-mysql-study/assets/91787050/7a69711f-193d-40fc-92d9-7d008b541660)


`PK`가 없는 테이블은 어떻게 `클러스터링 테이블`로 구성될까?

`PK`가 없는 경우에는 다음 우선순위대로 `PK`를 대체할 컬럼을 선택한다.

1. `PK`가 있으면 기본적으로 `PK`를 **클러스터링 키**로 선택한다.
2. `NOT NULL` 옵션의 `UNIQUE INDEX` 중에서 첫 번째 인덱스를 **클러스터링 키로 선택**한다.
3. 자동으로 `유니크`한 값을 가지도록 증가되는 `컬럼`을 내부적으로 추가한 후, **클러스터링 키로 선택**한다.

`InnoDB 스토리지 엔진`이 적절한 `클러스터링` 키 후보를 찾지 못하면, `스토리지 엔진`이 **내부적으로 레코드의 일련번호 컬럼을 생성**한다.

자동으로 추가된 `PK`는 사용자에게 노출되지 않고 `쿼리 문장`에서 사용할 수 없다.

이렇게 추가되는 `일련변호`는 아무 의미 없는 숫자 값으로 `클러스터링` 되는 것이기에 아무런 도움이 되지 않는다.

따라서, `InnoDB 테이블`을 사용 중이라면 테이블 당 하나만 가질 수 있는 **PK를 명시적으로 생성**하자.

### 🫡 세컨더리 인덱스에 미치는 영향

`MyISAM`이나 `MEMORY` 테이블 같은 클러스터링되지 않은 테이블은 `INSERT`될 때 처음 저장된 공간에서 절대 이동하지 않는다.

`MyISAM` 테이블과 `MEMORY` 테이블에서는 프라이머리 키와 세컨더리 인덱스는 **구조적으로 아무런 차이가 없다.**

`프라이머리 키`나 `세컨더리 인덱스`의 각 키는 주소**(ROWID)를 이용해 실제 데이터 레코드를 찾아오는 방식**이 동일하기 때문이다.

`InnoDB 테이블`의 모든 `세컨더리 인덱스`는 해당 레코드가 저장된 주소가 아니라 `프라이머리 키` 값을 저장하도록 구현되어 있다.
...
위와 같은 `테이블`이 있다고 할때, `세컨더리 인덱스`를 이용해 데이터를 조회할 때 `MyISAM` 과 `InnoDB`가 어떻게 동작할까?

```sql
mysql> SELECT * FROM employees WHERE first_name='Aamer';
```

- `MyISAM` : ix_firstname 인덱스를 검색해서 레코드의 주소를 확인한 후, `레코드의 주소`를 이용해 최종 레코드를 가져온다.
- `InnoDB` : ix_firstname 인덱스를 검색해 레코드의 PK 값을 확인한 후 `PK 인덱스를 검색`해서 최종 레코드를 가져온다.

`InnoDB`가 `MyISAM` 보다 조금 더 복잡하게 처리된다.

하지만 `InnoDB` 테이블에서 더 큰 장점을 제공하기 때문에 이 부분에 대한 `성능 저하`를 너무 걱정할 필요는 없다.

### 😔 클러스터링 인덱스의 장점과 단점

`MyISAM`과 같은 클러스터링되지 않은 일반 `프라이머리 키`와 `클러스터링 인덱스`를 비교했을 때의 상대적인 장단점은 어떤 것이 있을까?

![image](https://github.com/AK-47-Study/real-mysql-study/assets/91787050/d8a0cdf7-2c1f-4f9f-939c-b5a4754172cb)

`클러스터링 인덱스`의 장점은 **빠른 읽기**라는 것을 알 수 있다.

일반적인 `웹 서비스`나 `온라인 트랜잭션` 환경에서는 조금 느린 쓰기를 감수하고 **읽기를 빠르게 유지하는 것이 매우 중요**하다.

### 😃 클러스터링 인덱스 키의 크기

- `클러스터링 테이블`은 모든 **세컨더리 인덱스가 PK값을 포함**한다.
- `PK`의 크기가 커지면 `세컨더리 인덱스`도 자동으로 크기가 커진다.

일반적으로 테이블에 `세컨더리 인덱스`가 4~5개 정도 생성된다는 것을 고려하면 `세컨더리 인덱스` **크기는 급격히 증가**한다.

![image](https://github.com/AK-47-Study/real-mysql-study/assets/91787050/fdd1a566-bda8-4c67-9225-2a4b2c4f35b9)

레코드 한 건 단위로 생각해보면 `50바이트` 쯤은 큰 사이즈가 아니기 때문에 괜찮을 거 같아 보인다.

하지만, 레코드 건수가 `100만 건`만 넘어가도 인덱스의 크기가 **기하급수 적으로 늘어난다.**

`인덱스`가 커질수록 같은 성능을 내기 위해 그만큼의 메모리가 더 필요하기 때문에 `InnoDB 테이블`의 `PK`는 **신중하게 선택**해야 한다.

### 🤓 PK는 가능하다면 업무적인 컬럼으로 생성

- `InnoDB`의 `PK`는 클러스터링 키로 사용되며, 이 값에 의해 레코드의 위치가 결정된다.
- `PK`는 의미만큼이나 `중요한 역할`을 하므로 대부분 검색에서 상당히 빈번하게 사용된다.
- `컬럼`의 크기가 크더라도 업무적으로 해당 `레코드`를 `대표`할 수 있는 `컬럼`이 있다면 그 `컬럼`을 `PK`로 설정하는 것이 좋다.

### 🫠 PK는 반드시 명시할 것

- `AUTO_INCREMENT` 컬럼을 이용해서라도 `PK`는 반드시 생성하는 것이 좋다.
- 사용자가 사용할 수 있는 값을 `PK`로 `설정`하는 것이 좋다.
- `ROW` 기반 복제나 `InnoDB Cluster`에서 모든 테이블이 `PK`를 가져야 정상적인 `복제 성능`을 `보장`하기도 하므로 `PK`는 꼭 `생성`해야 한다.

### 😎 AUTO_INCREMENT 컬럼을 인조 식별자로 사용할 경우

- 여러 개의 `컬럼`이 복합적으로 `PK`가 만들어지는 경우 `PK` 키의 크기가 커지는 경우가 있다.
- `PK` 키의 크기가 길어도 `세컨더리 인덱스`가 필요하지 않다면 그대로 사용해도 좋다.
- `세컨더리 인덱스`도 필요하고, `PK` 키의 크기도 길면 **인조 식별자를 사용**하는 것이 좋다.
- `INSERT` 위주의 테이블들은 `AUTO_INCREMENT`를 이용한 `인조 식별자`를 PK로 설정하면 **성능 향상에 도움**이 된다.

### 🙆🏼‍♂️ 유니크 인덱스

- `Unique`는 **인덱스보다는 제약 조건**에 가깝다.
- `MySQL`은 인덱스 없이 `Unique` 제약만 설정할 방법이 없다.
- `MyISAM`이나 `MEMORY` 테이블에서 PK는 NULL이 허용되지 않는 `Unique` 인덱스와 같지만 `InnoDB`는 **클러스터링 키의 역할**도 하므로 근본적으로 다르다.

`유니크 인덱스`와 일반 `세컨더리 인덱스`는 인덱스의 구조상 아무런 차이점이 없다.

### ☹️ 인덱스 읽기

- `유니크 인덱스`는 무조건 빠르지 않다.
- 일반 `세컨더리 인덱스`에서 한번 더 해야 하는 작업은 디스크 읽기가 아니라 `CPU`에서 컬럼값을 비교하는 작업이므로 `성능상 영향`이 거의 없다.

`유니크`하지 않은 `세컨더리 인덱스`는 중복된 값이 허용되므로 **읽어야 할 레코드가 많아서 느린 것**이지, `인덱스`의 특정 때문에 `느린 것`이 아니다.

읽어야 할 레코드 건수가 같다면 `일반 세컨더리 인덱스`와 `유니크 인덱스`는 **성능상의 차이가 미미**하다.

### 🧑🏼‍🎄 인덱스 쓰기

- 새로운 레코드가 `INSERT` 되거나 인덱스 컬럼의 값이 `변경`되는 경우에는 **인덱스 쓰기 작업이 필요**하다.
- `유니크 인덱스`는 키 값을 쓸 때 중복된 값이 있는지 확인해야 해서 **한 단계 과정이 더 필요**하다.

`유니크 인덱스`의 쓰기 작업은 일반 세컨더리 인덱스보다 느리다.

`MySQL` 에서는 유니크 인덱스에서 **중복된 값을 체크**할 때 `읽기 잠금`을 사용하고, **쓰기 작업**에는 `쓰기 잠금`을 쓰는데 이 과정에서 `데드락`이 자주 발생한다.

`인덱스 키`의 저장을 버퍼링하기 위해서 `체인지 버퍼`를 사용해 **성능을 개선**했지만, `유니크 인덱스`는 중복 체크를 반드시 해야 하기 때문에 `작업 자체를 버퍼링` 할 수 없다.

### ☺️ 유니크 인덱스 사용 시 주의사항

- `성능`이 더 좋아질 것으로 생각하고 불필요하게 `유니크 인덱스`를 생성하는 것은 좋은 선택이 아니다.
- `유니크 인덱스`도 일반 세컨더리 인덱스와 같은 역할을 동일하게 수행할 수 있으므로 `세컨더리 인덱스`를 **중복으로 만들어 줄 필요는 없다.**
- `유일성`이 꼭 보장되어야 하는 `컬럼`에 대해서만 유니크 인덱스를 생성하고, 그렇지 않은 `컬럼`은 `일반 세컨더리 인덱스`를 **생성하는 방법을 고려**해보자.

### 😶‍🌫️ 외래키

- `MySQL`에서 외래키는 `InnoDB` 스토리지 엔진에서만 생성할 수 있다.
- `외래키 제약`이 설정되면 자동으로 연관되는 테이블의 컬럼에 `인덱스`까지 `생성`된다.

`InnoDB`의 외래키 관리에는 중요한 두 가지 특징이 있다.

- `테이블의 변경(쓰기 잠금)`이 발생하는 경우에만 잠금 경합이 발생
- `외래키`와 연관되지 않은 `컬럼`의 `변경`은 최대한 잠금 경합을 발생시키지 않는다.
...
mysql> INSERT INTO tb_parents VALUES (1, 'parent-1'), (2, 'parent-2');
mysql> INSERT INTO tb_child VALUES (100, 1, 'child-100');
```

위와 같은 `쿼리`가 실행될 때 `자식 테이블`의 `변경`이 잠금 대기를 언제 하고, `부모 테이블`의 `변경`은 언제 잠금 대기를 할까?

### 🥵 자식 테이블의 변경이 대기하는 경우

 ![image](https://github.com/AK-47-Study/real-mysql-study/assets/91787050/b5243710-2b40-435f-b23b-2eca0192942f)

`커넥션 1`과 `커넥션 2`의 쿼리의 실행 과정을 살펴보자.

1. `1번 커넥션`이 트랜잭션을 시작한다.
2. `부모 테이블`에서 id가 2인 레코드에 **UPDATE를 수행**한다.
3. `2번 커넥션`이 트랜잭션을 시작한다.
4. `자식 테이블`의 **외래키 컬럼인 pid를 2로 변경**하는 쿼리를 실행한다.
5. `자식 테이블`의 변경 작업은 `부모 테이블`의 **변경 작업이 완료될 때까지 대기**한다.
6. `1번 커넥션`의 작업이 **ROLLBACK이나 COMMIT**으로 `트랜잭션`이 `종료`되면 `2번 커넥션`이 **대기 중이던 작업이 수행**된다.

`InnoDB 외래키` 관리는 부모 테이블의 해당 레코드가 `쓰기 잠금`이 걸려있는 경우 해당 `쓰기 잠금`이 **해제될 때까지 기다리는 특징**이 있다.

### 😲 부모 테이블의 변경 작업이 대기하는 경우

![image](https://github.com/AK-47-Study/real-mysql-study/assets/91787050/5a172086-1971-4ddf-a2f1-8925f1a0a631)

1. `1번 커넥션`이 트랜잭션을 시작한다.
2. 부모 키 `“1”`을 참조하는 `자식 테이블`의 레코드를 변경 시도하면서 `쓰기 잠금`을 `획득`한다.
3. `2번 커넥션`이 트랜잭션을 시작한다.
4. `부모 테이블`이 **id가 1인 레코드를 삭제 시도**하면, `자식 테이블`의 레코드에 대한 **쓰기 잠금이 해제될 때까지 대기**한다.
5. `자식 테이블`의 UPDATE 작업이 처리 되고 `1번 커넥션`의 **트랜잭션이 종료**되면 `2번 커넥션`의 **작업이 수행**된다.

`자식 테이블`을 생성할 때 `ON DELETE CASCADE` 특성을 주었기 때문에 `부모 레코드`가 삭제되면 **자식 레코드도 동시에 삭제**되야 하므로, 위와 같이 처리된다.

`외래 키`를 물리적으로 생성할 때 이런 현상으로 `잠금 경합`이 생길 수 있기 때문에 **모델링에 신중**해야 한다.

`물리적인 외래키`의 고려 사항은 `체크 작업`이 아니라 체크를 위해 **연관 테이블에 읽기 잠금을 거는 것을 고려하는 것** 중요하다.

이런 `잠금`이 다른 테이블로 `확장`되어 **쿼리의 동시 처리에 영향을 줄 수 있다는 것도 잊지 말아야 한다.**
